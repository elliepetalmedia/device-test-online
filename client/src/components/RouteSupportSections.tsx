  ...
  return (
    <section className="space-y-4">
      <div className="space-y-1">
        <h2 className="font-orbitron text-base uppercase tracking-widest text-white">
          {title}
        </h2>
            ...
            >
              <Card className="flex h-full flex-col gap-2 border-primary/20 bg-black/20 p-4 transition-all duration-200 hover:border-primary/50 hover:bg-primary/5">
                <div className="flex-1 space-y-1">
                  <h3 className="font-orbitron text-sm text-white">
      ...
      <div className="flex gap-3">
        ...
        <div className="space-y-2">
          ...
          {callout.linkTarget && callout.linkLabel ? (
            <Link
              href={getRouteDefinitionByTarget(callout.linkTarget).path}
      ...
      <summary className="flex cursor-pointer list-none items-center justify-between gap-4 p-5">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-orbitron text-sm uppercase tracking-[0.2em] text-primary">
...
export function DiagnosticNextSection({ target }: { target: ModuleType }) {
  ...
  return (
    <section className="space-y-4">
      {workflowCta ? (
        <Link
          href={getRouteDefinitionByTarget(workflowCta.target).path}
        ...
        >
          <Card className="border-primary/20 bg-black/20 p-5 transition-all duration-200 hover:border-primary/50 hover:bg-primary/5">
            ...
            <h3 className="mb-2 font-orbitron text-base text-white">{workflowCta.label}</h3>
            <p className="text-sm leading-relaxed text-muted-foreground">
    ...
    <div className="space-y-6">
      ...
      {content.primaryCta ? (
        <Card className="border-primary/20 bg-surface p-6 text-center">
          <h2 className="mb-2 font-orbitron text-lg text-white">Ready to run another check?</h2>
          <p className="mb-5 text-sm text-muted-foreground">
            Return to the suite or jump straight into the next page that fits this topic.
          </p>
          <div className="flex flex-col items-center justify-center gap-3 sm:flex-row">
            <Link
              href={getRouteDefinitionByTarget(content.primaryCta.target).path}
              className="rounded bg-primary px-6 py-3 font-bold text-black transition-colors hover:bg-primary/80"
            >
              {content.primaryCta.label}
            </Link>
            {content.secondaryCta ? (
              <Link
                href={getRouteDefinitionByTarget(content.secondaryCta.target).path}
                className="rounded border border-secondary/30 px-6 py-3 font-bold text-foreground transition-colors hover:border-primary/40 hover:text-primary"
              >
